...
class Menu(ttk.Frame):
	def __init__(self, parent):
        ...
        # self.place(relx = 0, y = 0, relwidth = 0.3, relheight = 1) - left side is filled 30% of the window
		self.create_widgets()
...
class Main(ttk.Frame):
    def __init__(self, parent):
        ...
        Entry(self, 'Entry 3','Button 3','red')

class Entry(ttk.Frame):
    def __init__(self, parent, label_text, button_text, label_background):
        super().__init__(parent)

        label = ttk.Label(self, text = label_text, background = label_background, anchor='center', justify='center')
        button = ttk.Button(self, text = button_text)

        label.pack(expand = True, fill = 'both')
        button.pack(expand = True, fill = 'both', pady = 10)

        # entry = ttk.Entry(self, justify='center')
