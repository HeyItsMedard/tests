import json
import os
from utils import check_answer, restart_from_main
...
def get_topic_file(answer: int) -> list:
    ...
    """
    return read_from_file(json_file_names[answer-1])
...
def add_data():
    ...
    while True:
        ...
        # check if esc
            ...
            answer = get_topic_index()
            if answer == 0:
                return
            
            # gets topic data and filename
            topic_data = get_topic_file(answer)
            print(topic_data)
            topic_file = (f"{get_topic_name(answer)}.json")
            ...
            if num_vid == 1: # channel
                while True:
                    ...
                    # User writes content for creator
                        ...
                        video_dict = write_content_for_creator()
                        channel_dict["creator"] = channel_name
                        channel_dict["contents"] = video_dict
                        topic_data.append(channel_dict)
                        write_to_file(topic_file, topic_data)
                        print("Mentve.")
                        break
            
            if num_vid == 2: # video
                list_creators(topic_data)

                # User chooses a channel
                answer = input("Szám: ")
                answer = check_answer(len(topic_data), answer)
                if answer == 0:
        ...
        if num_vid != 0:
            ...
            if num_vid == 3: # topic
                ...
                if num_vid == 1: # creator
                    confirm_deletion(answer, topic_data, topic_data, topic_file) #szám, adat, fájl
            
                if num_vid == 2: # content
                    # gets chosen channel's content
                    creator_contents = topic_data[answer-1]['contents']
                    list_content(creator_contents)
                    answer = input("Szám: ")
                    if answer == 0: # esc
                        print("Visszatérés főmenübe...")
                        return
                    answer = check_answer(len(creator_contents), answer)
                    confirm_deletion(answer, creator_contents, topic_data, topic_file)
