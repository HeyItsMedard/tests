...
class Highscore:
    ...
    def highscore_lists():
        ...
        highscore_list = Database.read_from_file(f"highscores/{json_file_names[int(answer)-1]}")
        ...
        for i, player in enumerate(filedata):
            if player["name"] == player_name and player_name != "Anonymus": 
                ...
                if highscore > filedata[i]["points"]:
                    ...
                    filedata = Highscore.order_highscore(filedata)
    ...
    def write_highscore_if_eligible(topic_name: str, hard_difficulty: bool, highscore: int):
        ...
        highscore_data = Database.read_from_file(highscore_path)
        ...
        if len(highscore_data) == 10 and min_value < highscore:
            ...
            PlayerFound = Highscore.playerFound(highscore_data, highscore_path, highscore, player_name)
            if not PlayerFound:
                # if len(highscore_data) == 10 and min_value < highscore:
                highscore_data.pop(9)
                Highscore.newPlayer(player_name, highscore, highscore_path, highscore_data)
