from database import get_topic_index, get_topic_file, get_topic_name
from highscore import write_highscore_if_eligible
import random
...
def play():
    ...
    topic = get_topic_file(answer)
    topic_name = get_topic_name(answer)

    # Gets the list of content based on chosen topic, randomises order
    content_list = get_playlist(topic)
    ...
    hard_difficulty = get_difficulty()
    ...
    while len(content_list) >= 2:
        ...
        if FirstRound:
            ...
            f_creator = get_creator_of_content(content_list[first], topic)    
            FirstRound = False
        ...
        s_creator = get_creator_of_content(content_list[second], topic) 

        # Gets correct answer based on views
        correct_answer = (get_correct_answer(f_views, s_views))
        player_answer = ""
    ...
    if points >= 0:
        ...
        react_to_points(points, length_for_reaction)
        write_highscore_if_eligible(topic_name, hard_difficulty, points)